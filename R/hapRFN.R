samplesPerFeature <- function(X, samples = 0, lowerB = 0.0, upperB = 1000.0) {
  ...
  res <- .Call("samplesPerFeature", X, samples, lowerB, upperB, 
               PACKAGE="hapRFN")
...
readSamplesSpfabia <- function(X, samples = 0, lowerB = 0.0, upperB = 1000.0) {
  ...
  res <- .Call("readSamplesSpRfn", X, samples, lowerB, upperB,
               PACKAGE="hapRFN")
...
# build DgC Matrix
    ...
    filename1 <- paste(prefixPath,fileName,pRange,sparseMatrixPostfix,".txt",sep="")
...
# RFN call
    ...
    rfn_res <- train_rfn(X=X, n_hidden=p, n_iter=cyc, etaW=.1, etaP=.1, minP=1e-2, seed=0)
    ...
    res <- new("Factorization", parameters = list("rfn", cyc, p), n = n, p1 = p, p2 = p, l = l, center = as.vector(1), scaleData = as.vector(1), X = as.matrix(1), L = noL, Z = nZ, M = as.matrix(1), LZ = as.matrix(1), U = as.matrix(1), avini = avini, xavini = xavini, ini = ini, Psi = myPsi, lapla = mylapla)
